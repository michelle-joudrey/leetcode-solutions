class Solution {
    ...
    {
        // candidates will be positive numbers
        var lastCandidate = -1
        for (index, candidate) in candidates.enumerated() {
            // Enforce that all newly created edges are unique
            ...
            if sum + candidate < target {
                ...
                // and that we can't reuse the current candidate 
                let remainingCandidates = candidates.suffix(from: candidates.startIndex + index + 1)
